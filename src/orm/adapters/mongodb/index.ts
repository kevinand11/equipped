...
export class MongoDbOrm implements Orm<MongoDbRepoConfig> {
	...
	private getCollection(table: MongoDbRepoConfig): Collection {
		return this.client.db(table.db).collection(table.col)
	}

	async findMany(schema: AnySchema, table: MongoDbRepoConfig, queryAst: QueryAST): Promise<Record<string, unknown>[]> {
	...
	async findOne(schema: AnySchema, table: MongoDbRepoConfig, queryAst: QueryAST): Promise<Record<string, unknown> | null> {
	...
	async deleteOne(schema: AnySchema, table: MongoDbRepoConfig, queryAst: QueryAST): Promise<Record<string, unknown> | null> {
		const collection = this.getCollection(table)
		const { filter } = compileMongoQuery(queryAst, schema.primaryKey)
