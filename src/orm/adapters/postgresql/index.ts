...
export class PostgresqlAdapter implements Adapter<PostgresqlRepoConfig> {
	...
	private getTableName(table: PostgresqlRepoConfig): string {
		if (table.schema && table.schema !== 'public') return `${table.schema}.${table.table}`
		return table.table
	}

	async findMany(schema: AnySchema, table: PostgresqlRepoConfig, queryAst: QueryAST): Promise<Record<string, unknown>[]> {
	...
	async findOne(schema: AnySchema, table: PostgresqlRepoConfig, queryAst: QueryAST): Promise<Record<string, unknown> | null> {
	...
	async deleteOne(schema: AnySchema, table: PostgresqlRepoConfig, queryAst: QueryAST): Promise<Record<string, unknown> | null> {
		const limitedAst = { ...queryAst, limit: 1 }
		const results = await this.deleteMany(schema, table, limitedAst)
		return results[0] ?? null
	}

	async deleteMany(schema: AnySchema, table: PostgresqlRepoConfig, queryAst: QueryAST): Promise<Record<string, unknown>[]> {
		const tableName = this.getTableName(table)
