...
export type Table<Id extends IdType, T extends Model<Id>, Transform = T, Extras extends Record<string, unknown> = {}> = {
  ...
  updateMany: (
    filter: Filter<T>,
    values: UpdateInput<T>,
    options?: Options & { getTime?: () => Date }
  ) => Promise<Transform[]>;
  updateOne: (
    filter: Filter<T>,
    values: UpdateInput<T>,
...
export type UpdateInput<T extends Model<any>> = DistributiveOmit<
  UpdateFilter<CreateInput<T>>,
  '$setOnInsert'
>
...
export type BulkWriteOperation<T extends Model<any>> =
  ...
  | { op: 'upsert'; filter: Filter<T>; makeId?: (i: number) => string } & ({ insert: CreateInput<T> } | { insert: Partial<CreateInput<T>>, update: UpdateInput<T> })
