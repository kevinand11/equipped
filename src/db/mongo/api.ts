  ...
  const table: core.Table<IdType, Model, Entity, { collection: Collection<Model> }> = {
    ...
    findMany: async (filter, options = {}) => {
      const sortArray = Array.isArray(options.sort)
        ...
        : options.sort
          ? [options.sort]
          : []
      const sort = sortArray.map(
        (p) => [p.field, p.desc ? 'desc' : 'asc'] as [string, SortDirection]
      )
    ...
    insertMany: async (values, options = {}) => {
      ...
      const insertedData = await Promise.all(
        payload.map(async (data) => await table.findById(data[idKey] as any, options))
      )
    ...
    updateOne: async (filter, values, options = {}) => {
      const result = await table.updateMany(filter, values, options)
      return result.at(0) ?? null
    },
    ...
    deleteOne: async (filter, options) => {
      const result = await table.deleteMany(filter, options)
      return result.at(0) ?? null
    },
